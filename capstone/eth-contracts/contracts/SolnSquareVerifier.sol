...
contract SolnSquareVerifier is realHomes {
    ...
    function addSolution(bytes32 solutionId, uint256 tokenId, address solnAccount)
    public {
        _solutions.push(Solution({
            index: tokenId,
            solnAccount: solnAccount
        }));
        _uniqueSolutions[solutionId] = Solution({
            index: tokenId,
            solnAccount: solnAccount
        });
        emit SolutionAdded(solnAccount);
    }
    ...
    public {
        ...
        require(_uniqueSolutions[solutionId].solnAccount == address(0), "Solution already exists");
