...
contract('Flight Surety Tests', async (accounts) => {
    ...
    it('(airline) cannot register a flight that has old timestamp', async () => {
        ...
        try {
            await expectThrow(
                config.flightSuretyApp.registerFlight(airName, timestamp, {
                    from: config.firstAirline
                })
            );
        } catch (e) {
    ...
    it('(passenger) cannot buy insurance for a flight paying more than 1 ether', async () => {
        // ARRANGE
        let passAccount = accounts[7];
        let airName = "BC0002";
        let timestamp = Math.trunc(((new Date()).getTime() + 3 * 3600) / 1000);
        let amountPaid = web3.utils.toWei("1.1", "ether");
        // ACT
        try {
            await config.flightSuretyApp.registerFlight(airName, timestamp, {
                from: config.firstAirline
            });
            await expectThrow(
                config.flightSuretyApp.buyInsurance(config.firstAirline, airName, timestamp, {
                    from: passAccount,
                    value: amountPaid
                })
            );
    ...
    it('(passenger) can buy insurance for a flight paying up to 1 ether', async () => {
        ...
        await config.flightSuretyApp.buyInsurance(config.firstAirline, airName, timestamp, {
            from: passAccount,
            value: amountPaid
        });
        // ASSERT
        let result = await config.flightSuretyData.getAmountPaidByInsuree.call(passAccount, config.firstAirline, airName, timestamp, {
            from: passAccount
        });
    ...
    it('(passengers) receive credit for the insurance bought and only once', async () => {
        // ARRANGE
        let passAccount1 = accounts[8];
        let passAccount2 = accounts[9];
        let airName = "BC0004";
        let timestamp = Math.trunc(((new Date()).getTime() + 3 * 3600) / 1000);
        ...
        await config.flightSuretyData.creditInsurees(insuranceReturnPercentage, config.firstAirline, airName, timestamp, {
            from: config.owner
        });
        await expectThrow(config.flightSuretyData.creditInsurees(insuranceReturnPercentage, config.firstAirline, airName, timestamp, {
            from: config.owner
        }));
        fundsAfter = await config.flightSuretyData.getFund(config.firstAirline);
        // ASSERT
        // Passenger1
        let resultPassengerAmountPaid1 = await config.flightSuretyApp.getAmountPaidByInsuree.call(config.firstAirline, airName, timestamp, {
            from: passAccount1
        });
        assert.equal(resultPassengerAmountPaid1, passengerAmountPaid1);
        let resultPassengerAmountCredit1 = await config.flightSuretyData.getInsureePayoutCredits.call(passAccount1, {
            from: passAccount1
        });
        assert.equal(resultPassengerAmountCredit1, passengerAmountPaid1 * insuranceReturnPercentage / 100);
        // Passenger2
        let resultPassengerAmountPaid2 = await config.flightSuretyApp.getAmountPaidByInsuree.call(config.firstAirline, airName, timestamp, {
            from: passAccount2
        });
        assert.equal(resultPassengerAmountPaid2, passengerAmountPaid2);
        let resultPassengerAmountCredit2 = await config.flightSuretyData.getInsureePayoutCredits.call(passAccount2, {
            from: passAccount2
        });
    ...
    it('(passenger) can withdraw credits successfully', async () => {
        ...
        // ACT
        try {
            balanceBeforeTransaction = await web3.eth.getBalance(passAccount);
            await config.flightSuretyApp.withdrawCredits({
                from: passAccount,
                gasPrice: 0
            });
            balanceAfterTransaction = await web3.eth.getBalance(passAccount);
        } catch (e) {
            assert.fail(e.message);
        }
        // ASSERT
