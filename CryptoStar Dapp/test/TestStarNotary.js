...
it('can add the star name and star symbol properly', async() => {
    ...
    await instance.createStar('Nitya Star!', tokenId, {
        "from": accounts[0]
    });
    ...
    assert.equal(tokenName, 'Nitya Star Token');
    assert.equal(tokenSymbol, 'INR');
});
...
it('lets a user transfer a star', async() => {
    ...
    // 1. create a Star with different tokenId
    await instance.createStar('Nitya Star', tokenId, {"from": accounts[0]});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    // 1. create a Star with different tokenId
    await instance.createStar('A New Star', tokenId, {"from": accounts[1]});

    // 2. Call your method lookUptokenIdToStarInfo
    let name = await instance.lookUptokenIdToStarInfo(tokenId, {"from": accounts[1]});
